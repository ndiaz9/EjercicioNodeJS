...
let readData = (pathname, callback) =>{
    if(pathname == "/api/clientes"){
        axios.get(url_clientes).then(response => callback(response.data)).catch(error => console.log(error));
    }else if(pathname == "/api/proveedores"){
        axios.get(url_proveedores).then(response => callback(response.data)).catch(error => console.log(error));
    }
};
